...
namespace TSS.MeshCut
{
    public class MeshSlicer
    {
        ...
        private Material _sliceInsideMaterial;
        
        public MeshSlicer(float explodeForce, Material sliceInsideMaterial)
        {
            _explodeForce = explodeForce;
            _sliceInsideMaterial = sliceInsideMaterial;
        }
        ...
        public bool Slice(GameObject obj, Plane plane, out GameObject part1, out GameObject part2)
        {
            ...
            if (_explodeForce > 0.0001f)
            {
                if(part1.TryGetComponent(out Rigidbody rgbd1))
                {
                    rgbd1.AddForce(-plane.normal * _explodeForce * rgbd1.mass);
                }
                
                if(part2.TryGetComponent(out Rigidbody rgbd2))
                {
                    rgbd2.AddForce(plane.normal * _explodeForce* rgbd2.mass);
                }
            }
        ...
        public GameObject CreateMesh(List<MeshVertex> vertices, int[] triangles, int[] crossTriangles, GameObject origin)
        {
            ...
            mf.sharedMesh = mesh;
            CopyMeshMaterial(origin, obj, _sliceInsideMaterial);
            MeshVertex.WriteToMesh(ref mesh, vertices);
        ...
        public static void CopyMesh(GameObject obj, Material sliceInsideMaterial)
        {
            ...
            CopyMeshMaterial(obj, o, sliceInsideMaterial);
